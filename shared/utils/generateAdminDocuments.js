...
export const generateContractPDF = (order, settings) => {
    ...
    const signatureLabel = settings?.documents?.managerName ? `Signature & Cachet de ${settings.documents.managerName}` : `Signature & Cachet de ${settings?.companyName || 'le Vendeur'}`;
    doc.text(signatureLabel, pageWidth - 20, finalY + 40, { align: 'right' });

    if (settings?.documents?.stampUrl) {
        // Draw Stamp if available
        try {
            doc.addImage(settings.documents.stampUrl, 'PNG', pageWidth - 60, finalY + 45, 40, 40);
        } catch (e) {
    ...
    if (settings?.documents?.signatureUrl) {
        ...
        try {
            doc.addImage(settings.documents.signatureUrl, 'PNG', pageWidth - 60, finalY + 50, 40, 20);
        } catch (e) {
...
export const generateInvoicePDF = (order, settings) => {
    ...
    // Add RIB info from settings
    const rib = settings?.rib || {};
    doc.text(`Banque : ${rib.bankName || settings?.bankName || 'N/A'}`, 25, finalY + 37);
    doc.text(`IBAN : ${rib.iban || settings?.iban || 'N/A'}`, 25, finalY + 44);
    doc.text(`BIC : ${rib.bic || settings?.bic || 'N/A'}`, 25, finalY + 51);
    doc.text(`Titulaire : ${rib.titulaire || settings?.bankHolder || 'N/A'}`, 25, finalY + 58);
